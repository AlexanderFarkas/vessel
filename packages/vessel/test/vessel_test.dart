...
void main() {
  ...
  test("Scoped providers work with overrides", () {
    final provider = Provider((_) => 0);
    final overrider = Provider((_) => 42);

    final rootContainer = ProviderContainer(overrides: [provider.overrideWith(overrider)]);
    final childContainer = ProviderContainer(
      parent: rootContainer,
      overrides: [provider.scope()],
    );

    expect(childContainer.read(provider), equals(42));
  });
  test("Scoped factory providers work with overrides", () {
    final provider = Provider.factory((_, int value) => 0);
    final overrider = Provider.factory((_, int value) => 42);
    ...
    expect(childContainer.read(provider(2)), equals(42));
  });
}
